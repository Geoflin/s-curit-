  ...
  <?php 
  foreach ($pdo->query('SELECT dateSeanceBegin FROM seance_cinema1', PDO::FETCH_ASSOC) as $date) { ?>
  ...
  <?php if(isset($_POST['triSeanceBegin'])){?>
    <?php
 foreach ($pdo->query('SELECT FilmName FROM seance_cinema1 WHERE dateSeanceBegin= "'.$_POST['dateSeanceBeginTest'].'" ', PDO::FETCH_ASSOC) as $FilmSalle) {
