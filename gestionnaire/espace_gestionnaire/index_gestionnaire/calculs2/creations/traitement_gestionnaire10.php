...
foreach ($pdo_kinepolise_cinema2->query('SELECT DateSeanceBegin FROM `seance_cinema1` WHERE DateSeanceBegin LIKE "'.$fusion.'" AND SalleName LIKE "'.$SalleName.'" ', PDO::FETCH_ASSOC) as $search) {
...
foreach ($pdo_kinepolise_cinema2->query('SELECT SalleName FROM `seance_cinema1` WHERE `DateSeanceBegin` >= "'.$fusion.'" AND `DateSeanceBegin` <= "'.$DateFinSeance.'" AND `SalleName` = "'.$SalleName.'" ', PDO::FETCH_ASSOC) as $creneau) {
...
foreach ($pdo_kinepolise_cinema2->query('SELECT DateSeanceBegin, SalleName FROM seance_cinema1 WHERE Id=(SELECT max(Id) FROM seance_cinema1);')as $maxid) {
  ...
  $place_disponible = $Nombre_de_place['Nombre_de_place'];
        ...
        if (isset($doublonInt)<1){
          ...
          if (isset($Unix_DateSeanceBegin)){
                if ($pdo_kinepolise_cinema2->exec('INSERT INTO seance_cinema1 (FilmName, DateSeanceBegin, SalleName, place_disponible) VALUES ("'. $_POST['FilmName'] . '", "' . $fusionDateBegin_AjouterSeance . '", "' . $_POST['SalleName'] .'", "'. $place_disponible .'");') !== false){};
